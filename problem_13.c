...
//*****************************************************************************/
// Using python for problem 13 just feels like cheating.  Python does arbitrary
// precision math, so I'm not getting anything out of it.  I'm going to do 
// problem 13 in C for the challenge.  I haven't really written straight C in a
// very, VERY long time, so this should be interesting.
//*****************************************************************************/
int big_numbers[100][50] = {
...
int* split_integer(int integer) {
    int* buffer = malloc(3 * sizeof(int));
    int base = 0;
    for(int i = 2; i >= 0; i--) {
        base = (int) pow(10.0, (double) i);
        buffer[i] = integer / base;
        integer -= buffer[i] * base;
    }
    return buffer;
}

int main() {
    //Take all 100 of the big numbers and reverse their array order.  This should make it more
    //straight forward to code.
    for(int i = 0; i < 100; i++) {
        for(int j = 0; j < 25; j++) {
            big_numbers[i][j] = big_numbers[i][j] ^ big_numbers[i][49 - j];
            big_numbers[i][49 - j] = big_numbers[i][j] ^ big_numbers[i][49 - j];
            big_numbers[i][j] = big_numbers[i][j] ^ big_numbers[i][49 - j];
        }
    }
